...
PROJECT_ID=359
PRIVATE_TOKEN="xvuXe3MQyC5sJRzzmvF5"
...
DD_TITLE="合并请求通知"
DD_KEYWORD="部署服务报错"
DD_REASON="Merge_Request_未合并至主分支~"
...
if [ "$MR_RESULT" != "$EMPTY" ];then
  ...
  DATA="{\"msgtype\":\"markdown\",\"markdown\":{\"title\":\"[合并请求]\",\"text\":\"### [${DD_KEYWORD}] 部署失败， MIDDLE-END 部署tag：$CI_COMMIT_TAG 存在落后提交，请拉取最新代码到上线分支进行上线！ ${DD_AT_MOBILES_STRING}\"},\"at\":{\"atMobiles\":${DD_AT_MOBILES_ARRAY},\"isAtAll\":${DD_AT_ALL}}}"
  # DD
