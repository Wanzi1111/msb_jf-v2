  ...
  <div>
    <el-row class="search-container">
      ...
      <el-col v-for="(item, index) in dataSearchs" v-show="!(item.visible === false) && (!item.hide || globalHide)" :key="index" v-bind="getLayout(item)" class="search-item">
        ...
        <!--选择器：select-->
        <el-select v-if="item.type === 'select'" v-model="form[item.prop]" class="search-item-element" v-bind="item.attr" v-on="item.attr">
          <el-option v-for="(option, optionIndex) in item.options" :key="optionIndex" v-bind="option" />
        </el-select>
        <!--级联选择器：cascader-->
        <el-cascader v-if="item.type === 'cascader'" v-model="form[item.prop]" class="search-item-element" v-bind="item.attr" :props="item.props" :options="item.options" />
        <!--开关：switch-->
        ...
        <!-- 时间选择框：datePickerSelect -->
        <div v-if="item.type === 'datePickerSelect'" v-bind="item.attr" class="search-item-element">
          <div class="datePickerSelect">
            <el-input v-bind="item.attr">
              <el-select slot="prepend" v-model="form[item.inputSelect.prop]" class="search-item-element-input-select" v-bind="item.inputSelect.attr">
                <el-option v-for="(option, optionIndex) in item.inputSelect.options" :key="optionIndex" v-bind="option" />
              </el-select>
            </el-input>
