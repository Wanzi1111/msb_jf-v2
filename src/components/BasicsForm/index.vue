<template>
  <div>
    <el-form ref="basicsForm" :model="form" v-bind="attr" v-on="$listeners">
        ...
        <template v-for="(item, index) in dataForms">
          <el-col v-if="item.noShow ? false : true" :key="index" v-bind="item.colAttr">
            ...
            <el-form-item v-if="!item.render && !item.slot && !(item.visible === false )" :key="index" :class="formItemCls(item.type)" :prop="item.prop" :label="item.label" :rules="item.rules">
              ...
              <!--级联选择器：cascader-->
              <el-cascader v-if="item.type === 'cascader'" v-model="form[item.prop]" class="form-item-element" v-bind="{...item.attr, ...item}" />
              <!--开关：switch-->
              ...
              <!--时间范围选择器：timeRange-->
              <el-time-picker v-if="item.type === 'timeRange'" v-model="form[item.prop]" is-range arrow-control range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间" placeholder="选择时间范围" />
              <!--时间范围选择器：timeFrame 开始时间与结束时间互不联动版-->
              ...
              <!--上传媒体：upload-media-->
              <upload-media v-if="item.type === 'upload-media' && !item.many" v-model="form[item.prop]" :item="item" @upload="uploadApi" v-on="$listeners" />
              ...
              <!-- <div v-if="item.type === 'upload-media' && item.many">
                <div v-for="(media,key) in form[item.prop]" v-show="item.type === 'upload-media'" :key="key" style="margin-bottom: 20px">
                  <upload-media v-model="form[item.prop][key]" :item="item" :many="true" @upload="uploadApi" @mediaPush="form[item.prop].push({})" @deleteOne="form[item.prop].splice(key, 1)" />
                </div>
              </div> -->
              <!--提示：note-->
              <div v-if="item.note" class="note" :class="`${item.noteConfig.style || 'info'}`">
                ...
                <template v-if="item.noteConfig.type === 'tips'">
                  <span>{{ item.noteConfig.label }}：{{ getVal(item.noteConfig.keys) }}</span>
                  <p v-if="item.noteConfig.tips" style="color: #f56c6c">{{ item.noteConfig.tips }}</p>
                </template>
                ...
                <template v-if="item.noteConfig.type === 'link'">
                  <a v-bind="item.noteConfig.attr"><i v-if="item.noteConfig.icon" :class="item.slotnoteConfig.icon" /> {{ item.noteConfig.text }}</a>
                </template>
      ...
      <!-- 表单说明插槽 -->
      <slot :form="form" name="Tips" />
      <!-- 操作条渲染 -->
      <el-form-item v-if="!hideAction()" :class="formActionCls" label=" " style="width: 100%">
        <el-button v-if="!hideAction('submit')" size="small" :loading="loading" type="primary" :icon="defaultActionIcon('submit')" @click="defaultAction('submit')">{{ defaultActionLabel('submit') }}</el-button>
        <el-button v-if="!hideAction('reset')" size="small" :loading="loading" type="info" plain :icon="defaultActionIcon('reset')" @click="defaultAction('reset')">{{ defaultActionLabel('reset') }}</el-button>
        <el-button v-if="!hideAction('back')" size="small" :loading="loading" :icon="defaultActionIcon('back')" @click="defaultAction('back')">{{ defaultActionLabel('back') }}</el-button>
        <template v-for="(item, index) in moreActions">
          <el-button v-if="item.type === 'button' && !(item.visible === false)" :key="index" size="small" v-bind="item.attr" :loading="loadings[item.attr ? item.attr.loading : false] || loading" @click="item.click(form)">{{ item.label }}</el-button>
...
export default {
  ...
  methods: {
    getVal(e) {
      ...
      if (e && e.length) {
        e.forEach(i => {
          count += this.form[i.key]?.length | 0
          count += this.form[i.key]?.length ? i.exCount : 0
          let num = 0
          for (let index = 0; index < this.form[i.key]?.length; index++) {
            if (regString.includes(this.form[i.key][index])) {
              num++
            }
          }
          if (['4', '7'].includes(this.form.supplierName) && this.form.domesticForeign === '1') {
            count += num
          }
        });
      }
      if (['6'].includes(this.form.supplierName) && this.form.domesticForeign === '0' && this.form.messageType === '1') {
        count -= 4
      }
      return count
    },
