...
routerModules.keys().forEach(filePath => {
  const path = filePath.split('.')[1]
  const file = routerModules(filePath).default
  if (!blackRouterList.includes(path)) {
    routerList.push(file)
  }
})

routerList.sort((a, b) => a.index - b.index)
/**
 ...
 * meta : {
    ...
    activeMenu: '/example/user'  if set path, the sidebar will highlight the path you set
  }
 */
